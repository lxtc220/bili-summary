...
def preload_asr_model(progress_callback=None):
    ...
    with _model_lock:
        ...
        if progress_callback: progress_callback("正在初始化 sherpa-onnx ASR 引擎 (SenseVoiceSmall)...")
        ...
        try:
            ...
            # 模型路径
            base_dir = os.path.join(os.path.dirname(__file__), "model_cache", "sherpa-onnx-sense-voice-small")
            model_path = os.path.join(base_dir, "model.onnx")
            # 如果存在量化模型且在 CPU 上运行，可以考虑使用 model_q8.onnx
            # 但用户有 4060，优先使用标准模型 + CUDA
            ...
            # 创建识别器
            try:
                _asr_model_instance = sherpa_onnx.OfflineRecognizer.from_sense_voice(
                    model=model_path,
                    tokens=tokens_path,
                ...
                if progress_callback: progress_callback(f"CUDA 加速不可用，回退到 CPU: {cuda_error}")
                provider = "cpu"
                _asr_model_instance = sherpa_onnx.OfflineRecognizer.from_sense_voice(
                    model=model_path,
                    tokens=tokens_path,
            ...
            if progress_callback: progress_callback(f"sherpa-onnx ASR 引擎已就绪 (使用 {provider})")
            return _asr_model_instance
            
        except Exception as e:
            if progress_callback: progress_callback(f"sherpa-onnx ASR 引擎加载失败: {e}")
            raise e
...
def transcribe_audio(audio_path, progress_callback=None):
    ...
    try:
        ...
        # 2. 执行转录逻辑
        if progress_callback: progress_callback("正在启动 sherpa-onnx 引擎处理音频 (SenseVoice)...")
        
        try:
            # sherpa-onnx 的调用方式
            stream = model.create_stream()
            # 需要读取音频数据，sherpa-onnx 接受 waveform
            import soundfile as sf
            audio, sample_rate = sf.read(audio_path)
            # 转换为 16000Hz (如果不是的话)
            if sample_rate != 16000:
                # 简单重采样（如果是 librosa 的话更好，但我们没装）
                # 其实 sherpa-onnx 的 accept_waveform 会处理，但最好还是 16k
                pass
            ...
            # 获取结果
            full_text = clean_transcription_text(stream.result.text)
